...
export default function DownloadFlow({ initialFlow }: DownloadFlowProps = {}) {
  ...
  // flow: prop 우선, 없으면 레거시 ?mode= 또는 fortune 기본
  const [flow, setFlow] = useState<'fortune' | 'practice' | 'extras'>(initialFlow ?? 'fortune');

  // URL params 읽기 — orderId + (initialFlow 없는 경우) 레거시 ?mode=
  useEffect(() => {
    ...
    if (initialFlow) {
      // 라우트별 고정 플로우
      if (initialFlow === 'practice') setMode('practice');
      if (initialFlow === 'extras') {
    ...
    // 레거시 ?mode= 지원 (기존 딥 링크 하위 호환)
    const modeParam = params.get('mode');
    if (modeParam === 'fortune' || modeParam === 'practice') {
      setMode(modeParam);
      setFlow(modeParam);
    }
    if (modeParam === 'extras') {
  ...
  const needsZodiacInput = flow === 'fortune' && mode === 'fortune' && !user?.birthDate;
